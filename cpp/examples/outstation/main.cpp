...
void ReceiveAndSendSensorData(std::shared_ptr<IOutstation> outstation)
{
	try
	{
		...
		while (true)
		{
			...
			std::cout << "[INFO] Client connected to send sensor data." << std::endl;
	...
	{
		std::cerr << "[ERROR] Exception in ReceiveAndSendSensorData: " << e.what() << std::endl;
	}
}
...
int main(int argc, char* argv[])
{
	...
	std::thread sensorThread(ReceiveAndSendSensorData, outstation);
	sensorThread.join();
